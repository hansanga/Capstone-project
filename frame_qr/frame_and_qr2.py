...
def insert_frame2(result='sum1'):
    ...
    # 배경 이미지 불러오기
    background_path = os.path.join(prefix, "results", "28.png")
    ...
    # font_path = "D:\\Project\\Capstone-project\\results\\bookman old style.ttf"

    # # 날짜 추가
    # time = dt.datetime.now()
    # datestr = time.strftime('%Y/%m/%d')
    # dateFont = ImageFont.truetype(os.path.join(prefix, font_path), 30)
    # date_bbox = dateFont.getbbox(datestr)  # 경계 상자 가져오기
    # datemark = Image.new('RGBA', (date_bbox[2] - date_bbox[0], date_bbox[2] - date_bbox[0]), (0, 0, 0, 0))
    # datedraw = ImageDraw.Draw(datemark)
    # datedraw.text((0, 0), datestr, fill="#000000", font=dateFont)
    # datemark = datemark.rotate(90, expand=1)

    # # 날짜 위치 조정
    # final_img.paste(datemark, ((img_size[0]*2) + (mark_bbox[2] - mark_bbox[0]) - 50, 1000 - 50 - 10 - 10 - (date_bbox[2] - date_bbox[0])), datemark)
